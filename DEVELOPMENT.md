...
- Never hardcode surface/text hexes — use tokens (`BackgroundBrush`, `SurfaceBrush`, `Orb.ElevatedBrush`, `BorderBrush`, `Text*Brush`, `Orb.VeilBrush`); semantic tints (gold/red/green badges, Steam navy) stay hardcoded on purpose
...
- Never `StaticResource` a theme brush — snapshots don't follow theme/accent swaps; always `DynamicResource` (note: `GradientStop.Color` needs `*Color` keys, `Foreground`/`Background` need `*Brush` keys — mixing them crashes at render)
...
- `ColorAnimation.To` can't bind `DynamicResource` — the scrollbar hover uses an accent overlay with opacity animation instead
